...
@st.cache_data
def load_shapefile():
    cache_path = os.path.join(GEO, 'sa3_simplified.geojson')
    # Use cached simplified file if available
    ...
    if os.path.exists(shp):
        ...
        # Fetch from ABS ArcGIS REST API
        try:
            import requests, tempfile
            url = (
                "https://geo.abs.gov.au/arcgis/rest/services/ASGS2021/SA3/MapServer/0/query"
                "?where=1%3D1&outFields=SA3_CODE_2021%2CSA3_NAME_2021"
            ...
            gdf = gdf.rename(columns={'sa3_code_2021': 'sa3_code', 'sa3_name_2021': 'sa3_name_geo'})
            gdf['sa3_code'] = pd.to_numeric(gdf['sa3_code'], errors='coerce').astype('Int64')
        except Exception:
            return None
    # Simplify geometry and save cache
...
_sa3_filter   = sa3_sel if sa3_sel else sa3_opts

df = master[
    (master['year'] == year_sel) &
    (master['state'].isin(_state_filter)) &
    (master['mmm_code'].isin(_mmm_filter)) &
    (master['sa3_name'].isin(_sa3_filter))
].copy()

ratings_f = ratings[
    (ratings['state'].isin(_state_filter)) &
    (ratings['mmm_code'].isin(_mmm_filter)) &
    (ratings['sa3_name'].isin(_sa3_filter))
].copy()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Neighbourhood search — state then SA3 dropdowns
    st.markdown("**Check my neighbourhood**")
    _col1, _col2 = st.columns([1, 2])
    if not df.empty:
        states = sorted(df['state'].dropna().unique().tolist())
        sel_state = _col1.selectbox("State", ["select state"] + states, label_visibility="collapsed")
        if sel_state != "select state":
            sa3_options = sorted(df[df['state'] == sel_state]['sa3_name'].dropna().unique().tolist())
        else:
            sa3_options = []
        sel_sa3 = _col2.selectbox("SA3 region", ["select SA3"] + sa3_options, label_visibility="collapsed", disabled=(sel_state == "select state"))
        if sel_state == "select state":
            _col2.caption("Select a state first")
        if sel_sa3 != "select SA3":
            row = df[df['sa3_name'] == sel_sa3].iloc[0]
            st.markdown(
                f'<div class="nb-card">'
                f'<b>{sel_sa3}</b> ({row.get("state","")})'
                f'&nbsp;&nbsp;·&nbsp;&nbsp;'
    ...
    if gdf is not None and not df.empty:
        ...
        fig_map = px.choropleth_mapbox(
            ...
            color_continuous_scale=[[0, '#4A90D9'], [0.5, '#F5C842'], [1, '#E8521A']],
        ...
        if sel_sa3 != "select SA3":
            sel_geom = merged[merged['sa3_name'] == sel_sa3]
            if not sel_geom.empty:
                centroid = sel_geom.geometry.centroid.iloc[0]
                fig_map.add_trace(go.Scattermapbox(
                    lon=[centroid.x], lat=[centroid.y],
                    mode='markers+text',
                    marker=dict(size=16, color='#FF4136'),
                    text=[sel_sa3], textposition='top right',
                    textfont=dict(color='#FF4136', size=12),
                    showlegend=False, name=sel_sa3,
                ))
        theme(fig_map, height=520)
        st.plotly_chart(fig_map, use_container_width=True)
    elif not df.empty:
        st.warning("Shapefile not found — showing fallback scatter chart.")
        scatter_df = df.dropna(subset=['access_rate', 'quality_score'])
        fig_fb = px.scatter(
            scatter_df,
        ...
        if sel_sa3 != "select SA3":
            hl = scatter_df[scatter_df['sa3_name'] == sel_sa3]
            if not hl.empty:
                fig_fb.add_trace(go.Scatter(
                    x=hl['access_rate'], y=hl['quality_score'],
                    mode='markers+text',
                    marker=dict(size=16, color='#FF4136', symbol='star',
                                line=dict(width=2, color='white')),
                    text=[sel_sa3], textposition='top center',
                    textfont=dict(color='#FF4136', size=12),
                    showlegend=False, name=sel_sa3,
                ))
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        scatter_df2 = df.dropna(subset=['access_rate', 'quality_score']).copy()
        mmm_group = {'MM1': 'Metro', 'MM2': 'Metro', 'MM3': 'Regional', 'MM4': 'Regional',
                     'MM5': 'Remote', 'MM6': 'Remote', 'MM7': 'Remote'}
        scatter_df2['remoteness'] = scatter_df2['mmm_code'].map(mmm_group).fillna('Unknown')
        # Dominant org type per SA3
        def _dominant(row):
            counts = {'government': row.get('n_government', 0) or 0,
                      'not_for_profit': row.get('n_nfp', 0) or 0,
                      'profit': row.get('n_private', 0) or 0}
            return max(counts, key=counts.get)
        scatter_df2['dominant_org'] = scatter_df2.apply(_dominant, axis=1)

        sel_org = st.session_state.get('sel_org_type', None)
        org_label_map_s = {'profit': 'For profit', 'not_for_profit': 'Not for Profit', 'government': 'Government'}
        remote_colours = {'Metro': '#4A90D9', 'Regional': '#F5C842', 'Remote': '#E8521A'}

        if sel_org:
            fig_scatter2 = go.Figure()
            # Greyed-out traces first (background)
            grey_df = scatter_df2[scatter_df2['dominant_org'] != sel_org]
            fig_scatter2.add_trace(go.Scatter(
                x=grey_df['access_rate'], y=grey_df['quality_score'],
                mode='markers',
                marker=dict(size=7, color='#D0D0D0', opacity=0.3),
                hoverinfo='skip', showlegend=False, name='Other',
            ))
            # Highlighted traces per remoteness group (keeps legend)
            for rem, col in remote_colours.items():
                sub = scatter_df2[(scatter_df2['dominant_org'] == sel_org) & (scatter_df2['remoteness'] == rem)]
                if sub.empty:
                    continue
                fig_scatter2.add_trace(go.Scatter(
                    x=sub['access_rate'], y=sub['quality_score'],
                    mode='markers', name=rem,
                    marker=dict(
                        size=(sub['pop_65_plus'] ** 0.35).clip(6, 30),
                        color=col, opacity=1.0,
                        line=dict(width=1, color='white'),
                    ),
                    customdata=sub[['sa3_name', 'state', 'mmm_code', 'care_gap_index']].values,
                    hovertemplate='<b>%{customdata[0]}</b><br>State: %{customdata[1]}<br>Access: %{x:.1f}%<br>Quality: %{y:.2f}<br>CGI: %{customdata[3]:.2f}<extra></extra>',
                ))
            fig_scatter2.update_layout(
                title=f'Access rate vs quality score ({year_sel}) — {org_label_map_s.get(sel_org, sel_org)} highlighted',
                xaxis_title='Access rate (%)', yaxis_title='Quality score',
                legend_title='Remoteness',
            )
        else:
            fig_scatter2 = px.scatter(
                scatter_df2,
                x='access_rate', y='quality_score',
                color='remoteness', size='pop_65_plus',
                color_discrete_map=remote_colours,
                category_orders={'remoteness': ['Metro', 'Regional', 'Remote']},
                hover_name='sa3_name',
                hover_data={'state': True, 'mmm_code': True, 'care_gap_index': ':.2f'},
                title=f'Access rate vs quality score ({year_sel})',
    ...
    def _org_color(ot):
        if sel_org is None or ot == sel_org:
            return ORG_COLOURS.get(ot, C['teal'])
        return '#D0D0D0'
    ...
    if org_event and org_event.selection and org_event.selection.points:
        clicked_label = org_event.selection.points[0].get('y')
        label_to_type = {v: k for k, v in org_label_map.items()}
        clicked_type = label_to_type.get(clicked_label)
        if clicked_type:
            if st.session_state.get('sel_org_type') == clicked_type:
                st.session_state['sel_org_type'] = None
            else:
                st.session_state['sel_org_type'] = clicked_type
    ...
    with c2:
        ...
        for _, row in totals.iterrows():
            fig_fund.add_annotation(
                x=row['year'], y=row['funding_b'],
                text=f"<b>${row['funding_b']:.1f}B</b>",
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df.empty:
        ...
        fig_wi.add_annotation(
            x=whatif_target, y=1.22, yref='paper',
            text=f'Target: {whatif_target:.0f}',
            showarrow=True, ax=0, ay=20,
            arrowhead=0, arrowcolor=C['red'],
            xanchor='center',
            font=dict(color=C['red'], size=11, family='Arial Black'),
        )
        fig_wi.add_vline(x=med_beds, line_dash='dot', line_color=C['muted'])
        fig_wi.add_annotation(
            x=med_beds, y=1.10, yref='paper',
            text=f'Median: {med_beds:.1f}',
            showarrow=True, ax=0, ay=20,
            arrowhead=0, arrowcolor=C['muted'],
            xanchor='center',
            font=dict(color=C['muted'], size=11, family='Arial Black'),
        )
        theme(fig_wi, height=300)
        fig_wi.update_layout(margin=dict(t=65))
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        before = ratings_f[ratings_f['period'] == 'Before mandate'][dims].mean()
        after  = ratings_f[ratings_f['period'] == 'After mandate'][dims].mean()
        sub_df = pd.DataFrame({
            'Sub-rating': labels,
    ...
    with c2:
        org_time = (
            ratings_f[ratings_f['org_type'] != 'unknown']
    ...
    comp_time = (
        ratings_f[ratings_f['snapshot_date'] >= MANDATE]
